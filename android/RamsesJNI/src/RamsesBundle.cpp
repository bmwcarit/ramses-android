...
namespace ramses_bundle
{
    ...
    bool RamsesBundle::linkProperties(const rlogic::Property& sourceProperty, const rlogic::Property& targetProperty)
    {
        if (!m_logicEngine)
        {
            __android_log_print(ANDROID_LOG_ERROR, "RamsesNativeInterface", "Can't link properties as there is no logic engine!");
            return false;
        }

        return m_logicEngine->link(sourceProperty, targetProperty);
    }

    bool RamsesBundle::unlinkProperties(const rlogic::Property& sourceProperty, const rlogic::Property& targetProperty)
    {
        if (!m_logicEngine)
        {
            __android_log_print(ANDROID_LOG_ERROR, "RamsesNativeInterface", "Can't unlink properties as there is no logic engine!");
            return false;
        }
        return m_logicEngine->unlink(sourceProperty, targetProperty);
    }
}
